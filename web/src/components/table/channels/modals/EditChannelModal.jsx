...
const EditChannelModal = (props) => {
    ...
    if (success) {
      ...
      if (data.setting) {
        try {
          ...
          data.disable_non_streaming =
            parsedSettings.disable_non_streaming || false;
        } catch (error) {
          ...
          data.disable_non_streaming = false;
        }
      } else {
        data.force_format = false;
        data.thinking_to_content = false;
      ...
      if (data.settings) {
        try {
          ...
          data.skip_v1_prefix = parsedSettings.skip_v1_prefix || false;
      ...
      setChannelSettings({
        ...
        disable_non_streaming: data.disable_non_streaming || false,
      });
  ...
  const submit = async () => {
    ...
    const channelExtraSettings = {
      ...
      disable_non_streaming: localInputs.disable_non_streaming || false,
    };
    ...
    if (localInputs.type === 1 || localInputs.type === 14) {
      ...
      // 仅 OpenAI 渠道需要 store / safety_identifier / include_obfuscation / skip_v1_prefix
      if (localInputs.type === 1) {
        ...
        settings.skip_v1_prefix = localInputs.skip_v1_prefix === true;
      } else {
        settings.skip_v1_prefix = false;
      }
    ...
    delete localInputs.disable_non_streaming;
    ...
    delete localInputs.skip_v1_prefix;
            ...
            const advancedSettingsContent = (
              <div className='space-y-4'>
                ...
                <div className='pt-3'>
                  ...
                  {inputs.type === 1 && (
                    <Form.Switch field='skip_v1_prefix' label={t('跳过 /v1 前缀')} checkedText={t('开')} uncheckedText={t('关')} onChange={(value) => handleChannelOtherSettingsChange('skip_v1_prefix', value)} extraText={t('用于兼容非标准 API 路径的 OpenAI 兼容渠道，开启后请求路径将不再自动添加 /v1 前缀')} />
                  )}

                  <Form.Switch field='thinking_to_content' label={t('思考内容转换')} checkedText={t('开')} uncheckedText={t('关')} onChange={(value) => handleChannelSettingsChange('thinking_to_content', value)} extraText={t('将 reasoning_content 转换为 <think> 标签拼接到内容中')} />
                  <Form.Switch field='pass_through_body_enabled' label={t('透传请求体')} checkedText={t('开')} uncheckedText={t('关')} onChange={(value) => handleChannelSettingsChange('pass_through_body_enabled', value)} extraText={t('启用请求体透传功能')} />
                  <Form.Switch field='disable_non_streaming' label={t('禁用非流式请求')} checkedText={t('开')} uncheckedText={t('关')} onChange={(value) => handleChannelSettingsChange('disable_non_streaming', value)} extraText={t('开启后该渠道只接收流式请求，非流请求将返回403（用于避免上游因频繁新建连接触发限流/风控）')} />
