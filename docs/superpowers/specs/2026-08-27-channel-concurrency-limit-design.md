...
## 9. 验收标准（2026-08-27 实测）

- [x] AC-F3-1 渠道配置 limit=2 时，第 3 个并发请求获得 429 且 body 含 `channel_concurrency_exceeded` 与正确 limit；任一请求完成后新请求放行（E2E [200,200,429,429,429] → 恢复 200）
- [x] AC-F3-2 limit=0（默认）行为与现状完全一致（E2E 清零后 6 路并发全部 200）
- [x] AC-F3-3 超限请求未触达上游（E2E 上游命中计数 = 仅成功请求数）
- [x] AC-F3-4 结构上不可能发生渠道转移重试（检查点位于 distributor `c.Next()` 之前、relay 进入之前，代码位置确认）
- [x] AC-F3-5 在途名额占满期间新请求必拒（单测 gate 式占位：阻塞 handler 未结束前第 N+1 个被拒）
- [x] AC-F3-6 先增后判与失败释放的原子语义（`go test -race` 并发爆发测试：名额未释放期间精确通过=上限值）
- [x] AC-F3-7 store 故障降级放行且不要求释放（单测注入错误 store）
- [x] AC-F3-8 前端编辑抽屉可设置限额并保存（浏览器实测：字段渲染、回填、改 3 → PUT → 重开抽屉回填 3 → UI 值对 relay 生效 [3×200, 2×429]）
